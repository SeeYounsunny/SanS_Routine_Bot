...
| ✍️ 루틴 기록 | 아침/저녁 알림 메시지에 **답장**하거나 `/add` 로 오늘 루틴 추가 (오전/저녁 모두 같은 날 기록) |
| 📋 어제 루틴 번호 선택 | 어제 기록이 있으면 **번호(쉼표 구분)** 로 선택 가능. 새로 추가할 건 쉼표 뒤에 적으면 됨 (예: `1,3,요가 10분`) |
...
- **오늘 루틴을 추가하려면** → **`/add`** 를 입력하세요.
  - **어제 루틴이 있는 경우**: 봇이 어제 루틴 번호 목록을 바로 보여줍니다.  
    기존 건은 **번호를 쉼표(,)로 구분**해서 쓰고, 새로 추가할 게 있으면 **쉼표 뒤에** 적어서 **그 메시지에 답장**하면 됩니다. (예: `1,3,요가 10분`)
  - **어제 루틴이 없는 경우**: "오늘 루틴을 추가해주세요!" 메시지에 **답장**으로 적으면 저장됩니다.
- **아침 8시 / 저녁 9시** 알림이 오면 → 해당 알림 메시지에 **답장**하면 됩니다.  
  어제 루틴이 있으면 봇이 번호 선택 메시지를 보내고, 같은 방식으로 **번호(쉼표 구분) + 새 루틴** 답장하면 저장됩니다.

모든 기록은 **그날 루틴**으로 저장되며, 오전/저녁 구분은 "오전 기록"/"저녁 기록"으로만 표시됩니다.

### 명령어

| 명령어 | 설명 |
|--------|------|
| `/start` | 봇 안내. 오늘 루틴 추가는 `/add` 로 시작하세요. |
| `/add` | 오늘 루틴 추가 (어제 루틴이 있으면 번호 선택지가 바로 표시됨) |
| `/delete` | 오늘 작성한 루틴 전부 삭제 |
| `/reset 비밀번호` | 전체 데이터 초기화 (서버에 설정된 비밀번호 일치 시에만 실행) |
| `/search YYYY-MM-DD` | 해당 날짜의 내 루틴 조회 (예: /search 2025-03-15) |
| `/summary` | 오늘 전체 루틴 AI 요약 (ANTHROPIC_API_KEY 필요) |
| `/myroutine` | 나의 오늘 루틴 기록 확인 |
| `/weekstats` | 지난 7일 통계 (가장 많이 기록한 사람 TOP 3, 루틴 TOP 3) |
