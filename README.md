...
| 🌙 저녁 알림 (21:00 KST) | 단체방 리마인드. 루틴 입력은 **봇과 1:1**에서 `/add` |
...
- **단체방**에는 매일 **08:00** 아침 알림, **12:00** 점심 리마인드(오늘 입력한 사람별 루틴 공지), **21:00** 저녁 리마인드가 올라옵니다.

모든 기록은 **그날 루틴**으로 저장되며, `/today` 및 `/search`는 **오전/저녁 구분 없이 오늘 루틴 하나**로 보여줍니다. **루틴 기록 자세한 사용법은 `/routinehelp`, 출석체크 사용법은 `/attendancehelp` 를 입력하세요.**
...
| `/help` | 루틴 기록 설명서 표시 (/routinehelp) |
| `/routinehelp` | 루틴 기록 설명서 표시 |
| `/attendancehelp` | 출석체크 사용법 안내 |
