...
| 기능 | 설명 |
|------|------|
| 🌅 아침 알림 (08:00 KST) | **어제** 루틴(`/list` 형식) + **오늘도 함께 열심히 기록하자** 안내. 입력은 **봇과 1:1** `/add` |
| 🌙 저녁 알림 (20:00 KST) | **오늘**까지 입력된 루틴(`/list` 형식) + 미입력·추가 입력 **응원** + 1:1 안내 (배포 당일도 20:00 전 가동 시 당일부터 발송) |
| ✍️ 루틴 기록 | **봇과 1:1 대화**에서 `/add` 로만 입력 (단체방에서는 입력 불가, 1:1 유도 메시지 표시) |
| 🔒 개인 명령 | `/today`, `/myroutine`, `/delete`, `/search` 는 **1:1에서만** (단체방에서는 안내 메시지). 멤버 검증은 `/add` 와 동일 |
...
- **단체방**에는 매일 **08:00** 아침(어제 기록 + 오늘 응원), **20:00** 저녁(오늘 입력 현황 + 응원) 알림만 올라옵니다. (봇이 해당 시각 **이전**에 켜져 있어야 그날 스케줄이 잡힙니다.)

**개인 루틴 조회·삭제 (`/today`, `/myroutine`, `/delete`, `/search`)**  
- **봇과 1:1 대화에서만** 사용할 수 있습니다. 단체방에서 입력하면 1:1로 이동하라는 안내가 나옵니다.  
- **단체방 멤버만** 1:1에서 동작합니다 (`/add`, `/setname` 과 동일한 `TELEGRAM_CHAT_ID` 기준).

모든 기록은 **그날 루틴**으로 저장되며, `/today` 및 `/search`는 **오전/저녁 구분 없이 해당 날짜 루틴**으로 보여줍니다. **루틴 기록·명령 자세한 사용법은 `/help`, 출석체크 사용법은 `/attendanceguide` 를 입력하세요.**
...
| `/myroutine` | 내가 자주 사용하는 루틴 TOP 5 (1:1에서만) |
| `/delete` | 오늘 작성한 루틴 전부 삭제 (1:1에서만) |
| `/reset 비밀번호` | 전체 데이터 초기화 (서버 비밀번호 일치 시에만 실행) |
| `/search YYYY-MM-DD` | 해당 날짜의 내 루틴 조회 (1:1에서만, 예: /search 2025-03-15) |
...
- **23:00(KST) 세션 종료** 시 **정원(기본 24명)이 안 찼을 때만** 출석 현황 메시지에서 **버튼을 제거**하고, 단톡에 **세션 종료 + 「오늘도 수고하셨습니다!」** 안내를 보냅니다. (이미 정원 달성이면 23시 작업은 생략 — 100% 당시 축하 메시지만 유지)  
- 위 안내 전체 문구는 환경변수 `ATTENDANCE_SESSION_END_MESSAGE` 로 바꿀 수 있습니다. (줄바꿈은 `\n`)
