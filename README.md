...
| 🌙 저녁 알림 (20:00 KST) | 단체방 리마인드. 루틴 입력은 **봇과 1:1**에서 `/add` (배포·재시작 **당일**에도 20:00 전에 올라가 있으면 당일 저녁부터 발송) |
| 🌙 야간 루틴 모음 (23:30 KST) | 단체방에 당일 `/list` 형식 요약 + 굿나잇 문구 (기록 없으면 안내 메시지) |
...
- **단체방**에는 매일 **08:00** 아침 알림, **12:00** 점심 리마인드(오늘 입력한 사람별 루틴 공지), **20:00** 저녁 리마인드, **23:30** 당일 루틴 모음(`/list` 형식)이 올라옵니다. (봇이 해당 시각 **이전**에 켜져 있어야 그날 스케줄이 잡힙니다.)
